...
//    }
    ...
    public RestResult<SearchFileVO> searchFile(SearchFileDTO searchFileDTO) {
        ...
        Long currentPageLong = Long.parseLong(currentPage + "");
        Long pageCountLong = Long.parseLong(pageCount + "");
        String fileType = searchFileDTO.getFileType();
        IPage<FileListVO> fileList = null;
        if ("0".equals(fileType)) {
            fileList = userFileService.userFileList(null, null, searchFileDTO.getFileName(), currentPageLong, pageCountLong);
        } else {
            fileList = userFileService.getFileByFileType(Integer.valueOf(fileType), currentPageLong, pageCountLong, SessionUtil.getSession().getUserId(), searchFileDTO.getFileName());
        }
//        System.err.println(fileList.getRecords());
        List<SearchFileVO> searchFileVOList = new ArrayList<>();
        for (FileListVO record : fileList.getRecords()) {
            SearchFileVO searchFileVO = new SearchFileVO();
            searchFileVO.setUserFileId(record.getUserFileId()).setFileName(record.getFileName())
                    .setFileUrl(record.getFileUrl()).setFileSize(record.getFileSize()).setFilePath(record.getFilePath())
                    .setIsDir(record.getIsDir()).setExtendName(record.getExtendName());
            searchFileVOList.add(searchFileVO);
        }
        return RestResult.success().dataList(searchFileVOList, searchFileVOList.size());
    }
...
//        System.err.println(userFileIds);
        String[] fileIds = userFileIds.split(",");
        if(fileIds != null && fileIds.length > 0) {
            for (String fileId : fileIds) {
                userFileService.deleteUserFile(fileId, sessionUserBean.getUserId());
                fileDealComp.deleteESByUserFileId(fileId);
            }
        }
//        List<UserFile> userFiles = JSON.parseArray(batchDeleteFileDto.getUserFileIds(), UserFile.class);
